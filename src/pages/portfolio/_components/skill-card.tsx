...
export default function SkillCard({ img, name, url }: { img: string; name: string; url: string }) {
  ...
  return (
    ...
    !error && (
      <Link
        href={url}
        className={`bg-white relative w-11 h-11 md:w-20 md:h-20 flex justify-center items-center m-1 rounded-md`}>
        <div
          style={{ backgroundColor: data.lightVibrant }}
          className={`absolute top-0 opacity-30 hover:opacity-50 w-full h-full rounded-md`}
        />
